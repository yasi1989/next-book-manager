...
export async function addBookAction(values: z.infer<typeof AddFormSchema>): Promise<BookStateReturn> {
    try {
        ...
        if(values.file) {
            ...
            const filePath = `image/${uuidv4()}.${fileExt}`;
            const { error } = await supabase.storage.from("book-manager").upload(filePath, book);
    ...
    try {
        ...
        if(values.file) {
            if(existingBook.coverUrl) {
                const {error: deleteError} = await supabase.storage.from("book-manager").remove([existingBook.coverUrl]);
                if (deleteError) throw deleteError;
            }
            const book = values.file[0];
            const fileExt = book.name.split(".").pop;
            const filePath = `images/${uuidv4()}.${fileExt}`;
            const { error: uploadError } = await supabase.storage.from("book-manager").upload(filePath, book);
